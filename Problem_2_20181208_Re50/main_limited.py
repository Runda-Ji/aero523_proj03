    ...
    for i in range(1,Nx+1):
        for j in range(1,Ny+1):
            ...
            if u_ave > 0:
                ...
                if mesh['u'][i+1,j] - mesh['u'][i-1,j] == 0:
                    r_F = 1;
                else:
                    r_F = (mesh['u'][i,j] - mesh['u'][i-1,j])/(mesh['u'][i+1,j] - mesh['u'][i-1,j]);
            else:
                phi_u = (3*mesh['u'][i,j]   + 6*mesh['u'][i+1,j] - mesh['u'][i+2,j])/8;
                if mesh['u'][i,j] - mesh['u'][i+2,j] == 0:
                    r_F = 1;
                else:
                    r_F = (mesh['u'][i+1,j] - mesh['u'][i+2,j])/(mesh['u'][i,j] - mesh['u'][i+2,j]);
            F_convec = u_ave*phi_u;
            #-------------------------------------------------------------------------
            v_ave = 0.5*(mesh['v'][i,j] + mesh['v'][i,j+1]);
            if v_ave > 0:
                phi_v = (3*mesh['v'][i,j+1] + 6*mesh['v'][i,j]   - mesh['v'][i,j-1])/8;
                if mesh['v'][i,j+1] - mesh['v'][i,j-1] == 0:
                    r_G = 1;
                else:
                    r_G = (mesh['v'][i,j] - mesh['v'][i,j-1])/(mesh['v'][i,j+1] - mesh['v'][i,j-1]);
            else:
                phi_v = (3*mesh['v'][i,j]   + 6*mesh['v'][i,j+1] - mesh['v'][i,j+2])/8;
                if mesh['v'][i,j] - mesh['v'][i,j+2] == 0:
                    r_G = 1;
                else:
                    r_G = (mesh['v'][i,j+1] - mesh['v'][i,j+2])/(mesh['v'][i,j] - mesh['v'][i,j+2]);
            G_convec = v_ave*phi_v;
            #-------------------------------------------------------------------------
            mesh['F'][i,j] = F_convec - nu/d*(mesh['u'][i+1,j] - mesh['u'][i,j])*SMART(r_F);
            mesh['G'][i,j] = G_convec - nu/d*(mesh['v'][i,j+1] - mesh['v'][i,j])*SMART(r_G);
        ...
        for j in range(0,Ny+1):
            ...
            if (i>0 and i<Nx) and (j>0 and j<Ny):
                ...
                if v_ave > 0:
                    ...
                    if mesh['u'][i+1,j+1] - mesh['u'][i+1,j-1] == 0:
                        r_Hx = 1;
                    else:
                        r_Hx = (mesh['u'][i+1,j+1] - mesh['u'][i+1,j])/(mesh['u'][i+1,j+1] - mesh['u'][i+1,j-1]);
                else:
                    phi_u = (3*mesh['u'][i+1,j]   + 6*mesh['u'][i+1,j+1] - mesh['u'][i+1,j+2])/8;
                    if mesh['u'][i+1,j] - mesh['u'][i+1,j+2] == 0:
                        r_Hx = 1;
                    else:
                        r_Hx = (mesh['u'][i+1,j] - mesh['u'][i+1,j+1])/(mesh['u'][i+1,j] - mesh['u'][i+1,j+2]);
                Hx_convec = v_ave*phi_u;
                #-------------------------------------------------------------------------
                u_ave = 0.5*(mesh['u'][i+1,j+1] + mesh['u'][i+1,j]);
                if u_ave > 0:
                    phi_v = (3*mesh['v'][i+1,j+1] + 6*mesh['v'][i,j+1]   - mesh['v'][i-1,j+1])/8;
                    if mesh['v'][i+1,j+1] - mesh['v'][i-1,j+1] == 0:
                        r_Hy = 1;
                    else:
                        r_Hy = (mesh['v'][i+1,j+1] - mesh['v'][i,j+1])/(mesh['v'][i+1,j+1] - mesh['v'][i-1,j+1]);
                else:
                    phi_v = (3*mesh['v'][i,j+1]   + 6*mesh['v'][i+1,j+1] - mesh['v'][i+2,j+1])/8;
                    if mesh['v'][i,j+1] - mesh['v'][i+2,j+1] == 0:
                        r_Hy = 1;
                    else:
                        r_Hy = (mesh['v'][i,j+1] - mesh['v'][i+1,j+1])/(mesh['v'][i,j+1] - mesh['v'][i+2,j+1]);
            ...
            #For boundary nodes
                ...
                r_Hx = 1;
                r_Hy = 1;
            mesh['Hx'][i,j] = Hx_convec - nu/d*(mesh['u'][i+1,j+1] - mesh['u'][i+1,j])*SMART(r_Hx);
            mesh['Hy'][i,j] = Hy_convec - nu/d*(mesh['v'][i+1,j+1] - mesh['v'][i,j+1])*SMART(r_Hy);
    return 0;
...
def read_data(fname):
    mesh = np.load('data\\%s_data.npy' %fname);
    return mesh.item()
    
def post_processing(mesh,Re,Ny,f):
    global psi,xx,yy;
    f0, ax0 = plt.subplots(figsize=(f*2,4));
    plt.pcolor(mesh['u'].transpose(),cmap=plt.cm.jet);
    plt.colorbar();
    ax0.set_autoscalex_on(0);
    plt.xticks(np.arange(0, mesh['Nx']+1, step=50));
    #plt.close(f0);
    Nx = mesh['Nx'];
    Ny = mesh['Ny'];
    h = 1;
...
#------------------------------------------------------------------------------
    f1, ax1 = plt.subplots(figsize=(2*f,4));
    xx,yy = np.meshgrid(x, y);
    psi = np.zeros([Nx+1,Ny+1]);    
    for i in range(0,Nx+1):
        for j in range(1,Ny+1):
            psi[i,j] = psi[i,j-1] + mesh['u'][i+1,j]*d;
    plt.contour(xx, yy, psi.transpose(), 40,cmap=plt.cm.jet);
    plt.colorbar();
    plt.grid();
    ax1.set_autoscalex_on(0);
    plt.xticks(np.arange(0, 2*f+1, step=1));
    plt.title('Re=%d,$N_y$=%d,f=%d'%(Re,Ny,f));
    plt.xlabel('$x/h$',fontsize =12);
    plt.ylabel('$y/h$',fontsize =12);
    plt.savefig('figure\\Re%d_Ny%d_f%d_streamline.pdf' %(Re,Ny,f),dpi=150);
    plt.close(f1);
#------------------------------------------------------------------------------
    f2, ax2 = plt.subplots(figsize=(2*f, 4))
    pu_py = np.zeros(Nx+1);
    for i in range(1,Nx+2):
        pu_py[i-1] = 0.5*mesh['u'][i,1]/d;
    plt.plot(x,pu_py);
    plt.grid();
    ax2.set_autoscalex_on(0);
    plt.xlim([0,2*f]);
    plt.xticks(np.arange(0, 2*f+1, step=1));
    plt.title('Re=%d,$N_y$=%d,f=%d'%(Re,Ny,f));
    plt.xlabel('$x/h$',fontsize =12);
    plt.ylabel('$\partial u/\partial y$',fontsize =12);
    plt.savefig('figure\\Re%d_Ny%d_f%d_pu_py.pdf' %(Re,Ny,f),dpi=150);
    plt.close(f2);
    return 0;
    ...
    for i in range(0,1):
        for j in range(0,1):
            for k in range(0,1):
                ...
                mesh = read_data('Re%d_Ny%d_f%d'%(Re,Ny,f));
                post_processing(mesh,Re,Ny,f);
    return 0;
