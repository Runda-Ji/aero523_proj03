...
def post_processing(mesh,Re,Ny,f):
    Re = Re*2;
    global psi,xx,yy;
    f0, ax0 = plt.subplots(figsize=(f,2));
...
#------------------------------------------------------------------------------
    ...
    plt.savefig('figure\\Re%d_Ny%d_f%d_streamline.pdf' %(Re,Ny,f),bbox_inches='tight',dpi=150);
    plt.close(f1);
#------------------------------------------------------------------------------
    ...
    plt.savefig('figure\\Re%d_Ny%d_f%d_pu_py.pdf' %(Re,Ny,f),bbox_inches='tight',dpi=150);
    plt.close(f2);
    return x_reatt;

def comparsion(x_reatt):
    f1 = plt.figure(figsize=(8,6));
    Re_list = [100, 200, 400];
    for i in range(0,3):
        for j in range(0,2):
            x_r = x_reatt[i,j,:];
            Re = Re_list[i]*np.ones(3);
            if j == 0:
                plt.plot(Re,x_r,'rx');
            else:
                plt.plot(Re,x_r,'bx');
    plt.ylabel('Re-attachment length $x_r/h$',fontsize =12);
    plt.xlabel('Reynolds number',fontsize =12);
    plt.grid();
    plt.legend(['Ny=16','Ny=32']);
    plt.savefig('figure\\comparsion.pdf',dpi=150);
    plt.close(f1);
    return 0;

def main():
    global mesh,x_reatt,x_r;
    U0 = 1;
    h = 1;
    Re_list = [50, 100, 200];
    Ny_list = [16,32];
    f_list = [6,8,12];
    x_reatt = np.zeros([3,2,3]);
    x_r = [None]*3;
    for i in range(0,3):
        for j in range(0,2):
            for k in range(0,3):
                ...
                x_reatt[i,j,k] = post_processing(mesh,Re,Ny,f);
    comparsion(x_reatt);
    return 0;
